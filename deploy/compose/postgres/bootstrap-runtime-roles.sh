...
SQL
  ...
  runtime)
    ...
    vfs_password=$(read_hex_secret /run/secrets/vfs-database-password)
    headscale_sync_password=$(read_hex_secret /run/secrets/headscale-sync-database-password)
    {
      ...
      printf "\\set vfs_password '%s'\n" "${vfs_password}"
      printf "\\set headscale_sync_password '%s'\n" "${headscale_sync_password}"
      cat <<'SQL'
\set ON_ERROR_STOP on
BEGIN;
...
SELECT format('CREATE ROLE filebelt_vfs_login LOGIN INHERIT PASSWORD %L', :'vfs_password')
WHERE NOT EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'filebelt_vfs_login') \gexec
SELECT format('CREATE ROLE filebelt_headscale_sync_login LOGIN INHERIT PASSWORD %L', :'headscale_sync_password')
WHERE NOT EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'filebelt_headscale_sync_login') \gexec
...
ALTER ROLE filebelt_vfs_login PASSWORD :'vfs_password';
ALTER ROLE filebelt_headscale_sync_login PASSWORD :'headscale_sync_password';
...
GRANT filebelt_vfs TO filebelt_vfs_login;
GRANT filebelt_headscale_sync TO filebelt_headscale_sync_login;
COMMIT;
SQL
    } | psql --host postgres --username filebelt_owner --dbname filebelt --no-psqlrc
    psql --host postgres --username filebelt_owner --dbname filebelt --no-psqlrc \
