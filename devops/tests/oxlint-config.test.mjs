...
const FormatterConfig = JSON.parse(
  readFileSync(fileURLToPath(new URL('../../.oxfmtrc.json', import.meta.url)), 'utf8'),
)
...
test("enables FileBelt's repository-wide diagnostic-only style rules", () => {
  assert.equal(Config.rules['filebelt/no-semicolons'], 'error')
  assert.equal(Config.rules['filebelt/single-quotes'], 'error')
  assert.equal(FormatterConfig.semi, false)
  assert.equal(FormatterConfig.singleQuote, true)
  assert.equal(FormatterConfig.jsxSingleQuote, true)
  assert.ok(FormatterConfig.ignorePatterns.includes('ui/web/source/generated/openapi.ts'))
})
...
test('retains generated-source and runtime-global overrides', () => {
  assert.deepEqual(FindOverride('ui/web/source/generated/openapi.ts').rules, {
    ...
    'filebelt/no-semicolons': 'off',
    'filebelt/pascal-case': 'off',
    'filebelt/single-quotes': 'off',
  })
