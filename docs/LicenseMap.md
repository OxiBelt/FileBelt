...
| `adapters/directory-repository/` | Apache-2.0 | `@PiQuark6046` | Compatibility-disabled Apache wrapper in a separate workspace/process; no listener, image, bundled Git executable, or production grant exists in this release |
...
| `filebelt-smb-gateway` | `GPL-3.0-or-later` final image | Separate adapter workspace plus exact Samba `4.24.6` source/patch/build context. The scaffold pins the official archive SHA-256 and ships notices/source-offer requirements, but no image may publish until the complete corresponding source and working reviewed bridge are packaged. |
...
| `filebelt-directory-repository-adapter` scaffold | `Apache-2.0 AND MIT AND Unicode-3.0` executable | The isolated FileBelt-authored Apache wrapper links only the Apache directory-repository protocol and admitted permissive Rust dependencies. The compatibility release starts no listener, invokes no Git executable, has no image or deployment activation, and receives no production grants. Any later image must separately admit its exact Git executable and runtime composition. |
...
| `filebelt-wireguard-init` | `Apache-2.0 AND GPL-2.0-only AND MIT` final image | Isolated Apache wrapper plus separate static WireGuard tools `1.0.20260223` and iproute2 `7.1.0` GPL executables and musl runtime. Publication remains blocked until the exact checksum-pinned upstream archives, Cargo vendor closure, licenses/notices, build instructions, SBOMs, provenance, vulnerability decision, and corresponding-source bundle pass on every advertised native platform. |
| Tailscale transport image | Operator-integrated external image under its upstream terms | The private-egress chart pins the reviewed upstream GHCR manifest and does not copy or link Tailscale into an Apache image. Re-pin and review source, license, SBOM, provenance, supported architectures, control-plane terms, and state handling before qualification; SaaS and operator-provided Headscale remain external services. |
