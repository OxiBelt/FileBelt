...
| Interrupted one-time credential creation commits after recovery reports the operation missing, or arbitrary revokes exhaust PostgreSQL | PostgreSQL issues one expiring, generation-fenced creation slot per tenant/principal; create and recovery cancel lock that slot and cancel re-reads the credential after waiting; ordinary missing revocation is state-free | Bounded-row spam, prepare replay/rotation, stale/cross-principal denial, cancel-before-create, create-before-cancel, old-writer fail-closed, NFS compatibility, and least-privilege grant tests |
...
| Generic orphan cleanup deletes a live mount write or unexpired retained conflict | Generic selection requires finalized upload ownership and an active reservation, then PostgreSQL rechecks payload, upload, reservation, file-version, document-revision, and mount-writer authority before physical deletion; stale mount jobs restore live state or dispatch only through the fenced mount cleanup lease | Finalized live-writer exclusion, stale delete-intent restoration, retained-conflict preservation, final upload admission, and expired-writer mount-dispatch tests |
...
| Private tunnel becomes a generic proxy or crosses role/instance authority | Application reaches only a role-specific mTLS protocol gateway; gateway-to-relay ALPN and identity are distinct; relay receives no destination and dials only configured numeric same-port targets; every instance has exact NetworkPolicy selectors | Cross-role/SAN/ALPN/instance/port denial, malformed control, arbitrary destination, and direct-egress tests |
| Tunnel credential or state expands an application Pod | No application sidecar; Tailscale auth/state or WireGuard private key exists only in a dedicated relay Pod; userspace Tailscale has no TUN/capability and WireGuard grants only init-time `NET_ADMIN` | Rendered mounts/capabilities/RBAC, Pod inspection, credential absence, restart, and revocation tests |
...
| Concurrent first joins split one node across rooms or expose a uniqueness failure | The unique tenant/drive/node room row is the PostgreSQL convergence fence. One transaction creates the room and initial epoch; a conflicting transaction locks and reuses that committed room/epoch. Successful same-base or clean-epoch callers finalize independent one-use grants and receipts; a dirty base mismatch freezes the epoch and returns the existing conflict without retaining a pending receipt. | Deterministic two-caller PostgreSQL race, one-room/one-epoch convergence, distinct grants and receipts, dirty base-mismatch freeze, and no pending-receipt tests |
...
| Accepted third-party collaboration decoder risk affects an enabled room | Collaboration is disabled by default; operator opt-in accepts the tracked `yrs` `0.27.4` allocation risk under a single-target exact-identity fuzz quarantine. FileBelt rejects decoded zero-length GC blocks and maps Rust unwind panics from isolated snapshot/live decode, apply, and full-state re-encode to the existing typed rejection before persistence or ACK. The service-owned hook suppresses only marked containment panics and forwards unrelated panics; persisted rejection freezes the epoch. Valid sparse zero-clock states remain accepted. Process-aborting allocation failure through the unchanged top-level `clients_len` and `blocks_len` reservations remains accepted | Catalog identity and panic-hook ownership sentinels; zero-length and overflowing-range snapshot/live regressions; unchanged acknowledged-state, corrupt-freeze, and sparse-compatibility regressions; stable and ASan smoke; sustained sanitizer campaign plus reviewed snapshot/live-update regressions before clearance |
...
| Text or MCP semantic data executes markup or exhausts resources | filebelt-gfm-v1 renders raw HTML literally; strict UTF-8/no-NUL validation; server-persisted allowlisted 1-16 MiB edit and 8-100 MiB view bounds; Mermaid/KaTeX run only in reviewed, isolated render paths | XSS, malformed-UTF8, NUL, preference/size-bound, Mermaid/KaTeX, and semantic-output tests |
| A Git object ID, repository path, ref, hook, filter, or diff option becomes ambient authority or code execution | Commit IDs are identifiers only and every read reauthorizes; the adapter accepts opaque UUIDs and exact lowercase SHA-256 OIDs over mTLS, fixes the sole ref/tree entry/identity/message, disables ambient config/hooks/filters/protocols, and has no public Git transport or API credential | Cross-node/OID, path/ref injection, hook/filter/config, mTLS SAN, timeout/output, ref-CAS, and authorization-race tests |
| Authenticated revision comparisons exhaust coordinator tasks or system-Git processes | Immediate global and per-user coordinator admission before database/adapter work; a separate adapter private-task ceiling and immediate comparison Git-process admission; typed retryable overload with no partial diff; every terminal path releases permits | Cross-user fairness, global/per-user saturation, pre-spawn rejection, maximum concurrent-process, timeout, cancellation, error, and permit-reuse tests |
| Shared chunk reuse crosses drive or corrupts quota/reference state | PostgreSQL selects exact tenant/drive/digest/size reuse; 16 MiB immutable UUID locators and scoped capabilities expose no paths; manifests, physical quota, reference counts, delete intent, and quarantine commit under fences | Cross-drive digest collision, duplicate publish, quota, reference race, range-integrity, delete/restart, and scrub tests |
| Revision migration loses old history or activates a partial backfill | Same-transaction legacy-content trigger, per-version backfill jobs and repair holds, dual reads, PostgreSQL activation fence, Git fsck/ref and chunk-refcount verification, and v4 recovery inventory; no event-stream authority | Concurrent old writer, corrupt version, restart/retry, hold isolation, activation refusal, rollback, and restored v4 inventory tests |
| Preview frame, local Vite asset request, or converted Office document escapes its browser boundary | Opaque-origin `sandbox=allow-scripts` preview, parent Trusted Types policy `none`, child-only generated-markup policy and inline-style allowance for sanitized Mermaid/KaTeX output (never inline script), a data-free handshake that transfers a dedicated MessageChannel port for typed recursive-AST validation, canonical fixed-root local asset resolution with encoded namespace ownership and no static/SPA fallback, and `officeparser/slim` with OCR, attachments, and remote assets disabled | Chromium/Firefox iframe CSP, Trusted Types, postMessage/MessageChannel, encoded prefix/traversal, missing-asset/no-fallback, Office-format, warning, and size-bound tests |
...
| MCP mutation response loss duplicates or conceals an authoritative write | PostgreSQL-local handlers reserve, mutate, finalize the exact response, and commit once. Broker-mediated routes use a keyed stable operation UUID plus signed keyed request fingerprint; broker-local writes and safe result journal commit together, replay precedes rate/generation rejection, and API continuation plus public receipt commit together. Incomplete delete continuations are retained until API completion. API and maintenance receive only the exact receipt-column reads required by their fixed queries; maintenance deletes selected candidates by composite key and rechecks eligibility after acquiring each delete lock, with no table-level read or vault privilege. The broker retains only its journal lifecycle columns and cannot delete receipts. | Local/broker replay, cross-scope key mismatch, concurrent winner, rollback/no-pending-receipt, delete resumption, probe-result replay, broker-reset/purge race, OAuth secret exclusion, exact-role grant denial, and bounded-cleanup tests |
| Restore omits a revision backend, Git ref, shared chunk, vault generation, or MCP policy inventory | Recovery checkpoint v4 records canonical revision counts/digest, activation/holds/backfill state, purpose-bound keysets, MCP counts/tombstones, and referenced KEK generations without ciphertext; verification must match before traffic | Quiesced v4 checkpoint, missing Git/chunk/key, refcount/digest mismatch, unresolved hold, and fresh-target restore tests |
...
| Manager grants rights they do not have, overwrites a stale ACL, discloses an inaccessible node through pre-authorization ETag behavior, or removes a source-owned row | Resource-scoped existence-hiding `MANAGE_ACL` authorization precedes node/subject disclosure; the exact client ETag and authorization generations are revalidated under the PostgreSQL resource lock; strict attenuation covers submitted rows and mutable `core` rows removed by omission; owner, share, and non-core rows are read-only projections | Delegation and ACL-replacement tests, including all 26 actions, group round-trip, exact child depth, omitted deny/allow rows, empty clears, stale fences, inaccessible existing/missing equivalence, and owner/share/NFS preservation |
...
| Local deployment helper exposes a development service, deletes caller resources, or retains credentials | Named sessions own exact Compose resources or an isolated Minikube profile; loopback-only serving listeners, enforced Minikube quiescence, strict schema-v1 allowlists, explicit caller-qualified preview inputs, failure-only bounded redaction, and ownership-checked cleanup | Helper unit/contract tests; manual live deployment remains non-qualifying |
| Proxy or client retries after a resource mutation response is lost | OxiBelt write retries disabled; resource mutations reserve, mutate, serialize the exact response, and finalize the principal/route/key receipt in one PostgreSQL transaction; identifier-complete fingerprints reject cross-object key reuse | Lost-response, concurrent duplicate, cross-drive/node/upload key-reuse, and mutation-rollback tests |
...
| The qualified OxiBelt prerelease or native crypto input is substituted | Exact version/digest, annotated tag/release/source map, lockfile, SBOM, notice, vulnerability decision, sealed qualification, ELF, and three-platform evidence | Supply-chain and image tests |
...
| Retained OxiBelt evidence is forged, incomplete, replaced, or detached from the pinned web base and multi-platform index | Offline GitHub/Sigstore verification against a verifier-pinned retained trusted root and all four raw manifest subjects; admission schema contains no root selector or downgrade path; exact certificate/OIDC/source/runner policy; archive and member hashes for attempt-bound qualification and vulnerability decisions; exact 30-image, two-chart, and 12-manifest inventories; beta qualification must contain no stable aliases or waiver; direct Dockerfile/image-plan/notice binding; and exact index, child, source, role, platform, and target-CPU predicates | Root-replacement, schema-downgrade, archive-inventory, offline-signature, forged-signature, Dockerfile-drift, workflow-gate, and bundle-tamper tests |
| OxiBelt accepts an unsafe or unusable rendered TLS identity | Every certificate path is relative to `/etc/oxibelt/cert`; public and per-upstream Secrets are mounted below that root; each outbound identity uses the native nested `client_identity` contract; world-readable, missing, and certificate/key-mismatched client identities fail `oxibelt --check` in native AMD64 and ARM64 image lanes | Helm render contracts and native configuration checks |
...
| A version sentinel or unsupported Phase 8 endpoint is presented as qualification | Exact seven-role evidence catalog, source revision and instance binding, executable success/failure assertions, positive samples, completed cleanup, and non-accepted prerequisite-bearing skips; compatibility advertisement requires the matching result | Evidence-validator, advertisement-binding, local endpoint, negative assertion, cleanup, and unsupported-prerequisite tests |
...
| Forged NFS handle, replay, or stale gateway returns revoked data or commits bytes | Dedicated rotating handle MAC; current/previous key only; export/node/generation binding; ordinary replay re-enters current session, handle/resource, and Virtual ACL admission before stored fields; only an exact applied no-data `EndSession` acknowledgement survives its own close while external authority remains current; PostgreSQL gateway epoch, expected head, quota, and authorization recheck before immutable commit | Handle fuzzing, key rotation, restart/grace, slot replay under session/credential/mapping/policy/ACL/feature/export/gateway revocation, post-close read denial, narrow `EndSession` replay, stale epoch, and expected-head tests |
| Untrusted fuzz input exhausts CI or a crash artifact discloses input data | Cataloged per-target input maximum, 10-second per-input timeout, 3 GiB RSS and 512 MiB allocation ceilings, ASan-only runner-owned fallible-allocation null returns, bounded smoke/campaign duration, ephemeral corpora/raw logs, sanitizer summaries and crash SHA-256 only | Catalog/runner contracts, stable and ASan smoke matrices, private regression review |
| A partial or substituted image archive enters Docker integration | Current-revision/channel image plan, exact AMD64 tag/checksum/metadata/evidence/SBOM/validation/smoke/vulnerability checks before load; consumers require successful producer jobs and never rebuild FileBelt | Image-evidence unit tests, workflow-integrity tests, exact-artifact Docker units |
| Concurrent Docker runs replace tags or retain another run's state | Unique project, fixture tags and disposable state; refusal to replace FileBelt/fixture tags; owned-image tracking and deterministic network/volume/state cleanup | Lifecycle refusal and cleanup tests, isolated core/collaboration/MCP jobs |
| The Docker acceptance publication becomes an egress or secret-bearing pivot | Only a read-only, unprivileged, capability-dropped raw TCP fixture with no secrets, configs, or storage joins the ordinary publication network; it bounds connections and forwards only to the web edge on the internal network. Loopback is the default and all automated runners require it; an operator-selected non-loopback bind requires an exact acknowledgement that the bundled OIDC issuer is passwordless. All FileBelt services remain unpublished. | Relay unit tests, rendered Compose boundary checks, and live core/collaboration/MCP outside-topology units |
| The synthetic MCP integration fixture becomes a private-address bypass | Production broker remains public-WebPKI port 443; one exact `.example.test` host is intercepted only inside the mTLS test fixture, every other integration-profile target fails closed, and no production/Kubernetes trust profile changes | Fixture policy tests and hostile MCP Docker unit; public DNS/second-hop TLS remains unqualified |
...
| A Git path/ref, repository ID, device token, or SSH key bypasses FileBelt ACL or crosses a directory-root boundary | Opaque root IDs, principal resolution, current root ACL/generation rechecks, no nested roots or cross-boundary moves, `.git` rejection, no host path in public/private contracts, and PostgreSQL-authoritative root membership | Cross-tenant/root, revoked-token/key, stale-generation, `.git`, nesting, move, and existence-hiding tests |
| Malicious receive-pack, tree, blob, LFS object, or commit fan-out exhausts coordinator, adapter, quota, or recovery resources | Reject packs above 1 GiB, pushes with more than 32 newly admitted first-parent commits, commits with more than 10,000 changed paths, trees with more than 100,000 entries, ordinary blobs above 100 MiB, and LFS objects above the configured max-file limit; reserve quota and validate before `main` projection | Limit-boundary, cancellation, quota, parser, process, and no-partial-projection tests |
| Git or LFS state disagrees with FileBelt versions after a crash, rollback, or garbage collection | Fenced PostgreSQL operation/journal, `main`-only projection, deterministic derived versions, reconciliation holds, 30-day committed-unreachable retention, 24-hour rejected/quarantine retention, and quiesced checkpoint-v5 verification | Crash-at-every-boundary, replay, fsck, retention, purge, checkpoint/restore, and rollback tests |
| Unqualified NFS, SMBv3, or FTPS write behavior creates a Git commit with ambiguous remote durability | Keep all mount writes disabled; qualify each protocol's exact save boundary separately (NFS `COMMIT`/final dirty `CLOSE`, SMB no durable handles, FTPS no resume) before allowing a directory-Git projection | Protocol-specific live client, replay, disconnect, revocation, and cross-writer acceptance tests |
