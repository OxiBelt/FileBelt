...
| Interrupted one-time credential creation commits after recovery reports the UUID missing | Caller-selected UUID has a durable PostgreSQL operation fence; create and revoke lock the same row, and missing revoke commits cancellation before returning not found | Revoke-before-create rejection, create/revoke lock ordering, committed revocation, and least-privilege grant tests |
