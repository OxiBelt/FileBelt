...
| The Docker acceptance publication becomes an egress or secret-bearing pivot | Only a read-only, unprivileged, capability-dropped raw TCP fixture with no secrets, configs, or storage joins the ordinary publication network; it bounds connections, forwards only to the web edge on the internal network, and publishes exactly one IPv4 loopback port, selected from the runner-owned range for `outside`; all FileBelt services remain unpublished | Relay unit tests, rendered Compose boundary checks, and live core/collaboration/MCP outside-topology units |
