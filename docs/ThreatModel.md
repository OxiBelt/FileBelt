...
| Untrusted fuzz input exhausts CI or a crash artifact discloses input data | Cataloged per-target input maximum, 10-second per-input timeout, 3 GiB RSS and 512 MiB allocation ceilings, bounded smoke/campaign duration, ephemeral corpora/raw logs, sanitizer summaries and crash SHA-256 only | Catalog/runner contracts, stable and ASan smoke matrices, private regression review |
