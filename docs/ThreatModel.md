...
| Accepted third-party collaboration decoder risk affects an enabled room | Collaboration is disabled by default; operator opt-in accepts the tracked `yrs` `0.27.3` risk under a single-target exact-identity fuzz quarantine, with existing ceilings and smoke coverage retained | Catalog identity sentinel, stable and ASan smoke, and a sustained sanitizer campaign plus reviewed snapshot/live-update regressions before clearance |
