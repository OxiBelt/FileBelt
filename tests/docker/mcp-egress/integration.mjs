...
export const IntegrationPaths = new Set(["/mcp", "/credential", "/malformed", "/oversized", "/redirect", "/session-confusion", "/slow"]);

export function IsIntegrationTarget(Target, Port, TrustProfile, IntegrationHost) {
  return IntegrationHost.length > 0
...
export function IntegrationResponse(RequestValue, Pathname, MaximumResponseBytes, GatewayPath = "/") {
  ...
  if (Method === "initialize") {
    ...
    return {
      Body: { jsonrpc: "2.0", id: Id, result: { protocolVersion: "2026-07-28", capabilities: { tools: {} }, serverInfo: { name: "FileBelt bounded integration fixture", version: "1" } } },
      ...(Pathname === "/slow" ? { DelayMs: 6_000 } : {}),
      Session: IntegrationSession,
    };
  }
  if (Method === "tools/list") return { Body: { jsonrpc: "2.0", id: Id, result: { tools: [{ name: "echo", title: "Bounded echo", description: "Returns bounded synthetic text", inputSchema: { type: "object" }, annotations: { readOnlyHint: true } }] } } };
  if (Method === "resources/list") return { Body: { jsonrpc: "2.0", id: Id, result: { resources: [] } } };
  if (Method === "prompts/list") return { Body: { jsonrpc: "2.0", id: Id, result: { prompts: [] } } };
  if (Method === "tools/call") return { Body: { jsonrpc: "2.0", id: Id, result: { content: [{ type: "text", text: "bounded integration result" }] } } };
