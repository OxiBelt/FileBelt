...
test("integration synthesis is exact-host, port, profile, path, and query bound", () => {
  ...
  assert.equal(IsIntegrationTarget(new URL(`https://${Host}/mcp?escape=1`), 443, "integration", Host), false);
  assert.equal(IsIntegrationTarget(new URL(`https://${Host}/unknown`), 443, "integration", Host), false);
  assert.equal(IsIntegrationTarget(new URL("https://other.example.test/mcp"), 443, "integration", Host), false);
...
test("integration responses expose bounded normal and hostile behaviors", () => {
  ...
  assert.equal(IntegrationResponse({ id: 1, method: "initialize" }, "/malformed", 64).Raw.toString(), "{not-json");
  assert.equal(IntegrationResponse({ id: 1, method: "initialize" }, "/oversized", 64).Raw.length, 65);
  assert.equal(IntegrationResponse({ id: 1, method: "initialize" }, "/slow", 64).DelayMs, 6_000);
  assert.equal(IntegrationResponse({ id: 1, method: "initialize" }, "/redirect", 64).Redirect, RedirectLocation);
  assert.equal(IntegrationResponse({ id: 1, method: "initialize" }, "/redirect", 64, RedirectFollowPath).Session, IntegrationSession);
});

test("session confusion accepts the issued identity before injecting a distinct identity", () => {
  assert.equal(IntegrationResponse({ id: 1, method: "initialize" }, "/session-confusion", 64).Session, IntegrationSession);
  assert.equal(IntegrationSessionDenial("/session-confusion", "notifications/initialized", IntegrationSession), undefined);
  assert.equal(
