...
max_lines="${FILEBELT_RUST_SOURCE_LINE_LIMIT-750}"

if [[ ! "${max_lines}" =~ ^[1-9][0-9]*$ ]]; then
  printf \
    'error: FILEBELT_RUST_SOURCE_LINE_LIMIT must be a positive base-10 integer; got %q\n' \
    "${max_lines}" >&2
  exit 2
fi
...
file_list=""
if ! file_list="$(mktemp -- "${TMPDIR:-/tmp}/filebelt-rust-source-files.XXXXXX")"; then
