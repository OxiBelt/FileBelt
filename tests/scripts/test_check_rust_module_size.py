...
class RustModuleSizeScriptTests(unittest.TestCase):
    ...
    def run_checker(
        ...
        *arguments: str,
        ...
        environment.pop("FILEBELT_RUST_SOURCE_LINE_LIMIT", None)
        if line_limit is not UNSET:
            environment["FILEBELT_RUST_SOURCE_LINE_LIMIT"] = str(line_limit)
    ...
    def test_explicit_warn_mode_scans_adapters(self) -> None:
        self.write_rust_file("adapters/smb/src/oversized.rs", 2)

        result = self.run_checker("--warn", line_limit="1")

        self.assertEqual(result.returncode, 0, result.stderr)
        self.assertIn("adapters/smb/src/oversized.rs", result.stderr)
    ...
    def test_reports_violations_in_stable_path_order(self) -> None:
        self.write_rust_file("source/src/z-last.rs", 2)
        self.write_rust_file("adapters/smb/src/a-first.rs", 2)

        result = self.run_checker("--warn", line_limit="1")

        self.assertEqual(result.returncode, 0, result.stderr)
        first = result.stderr.index("adapters/smb/src/a-first.rs")
        last = result.stderr.index("source/src/z-last.rs")
    ...
    def test_missing_source_root_fails_closed(self) -> None:
        ...
        for relative_root in ("source/crates", "adapters"):
            with self.subTest(relative_root=relative_root):
                removed = self.repo_root / relative_root
