...
record=${repo_root}/supply-chain/oxibelt-admission-v1.json

command -v gh >/dev/null
command -v jq >/dev/null
python3 "${script_dir}/validate-oxibelt-admission.py" --repo-root "${repo_root}"
...
trusted_root=$(jq -er '.trustedRoot.path' "${record}")
...
for kind in index platform; do
  ...
  gh attestation verify "${repo_root}/${subject}" \
    ...
    --custom-trusted-root "${repo_root}/${trusted_root}" \
