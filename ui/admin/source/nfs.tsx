...
function IsNfsReauthenticationRequired(Cause: unknown): boolean {
  return (
    Cause instanceof NfsReauthenticationRequiredError ||
...
export function NfsAdminSurface({ Client }: { Client: NfsAdminClient }): ReactNode {
  ...
  if (Snapshot === null && ErrorMessage === null && !ReauthenticationRequired) {
    return (
      <section aria-busy="true" aria-label={Strings.nfsAdministration}>
        <Spinner label={Strings.nfsLoading} />
      </section>
    );
  }
...
export function NfsAdminOverviewView({
  ...
  const ConfirmedMutation = async (
    Operation: (Confirmation: string) => Promise<void>,
  ): Promise<void> => {
  ...
  return (
    <section aria-labelledby="nfs-admin-heading" className="fb-nfs-admin">
      <header className="fb-nfs-heading">
        <div>
          <p className="fb-eyebrow">
            <Network aria-hidden="true" size={18} /> {Strings.nfsNetworkStorage}
          </p>
      ...
      <section aria-labelledby="nfs-tenant-confirmation-heading" className="fb-nfs-card">
        ...
        <label htmlFor="nfs-tenant-confirmation">
          {Strings.nfsTenantConfirmationLabel(Snapshot.TenantSlug)}
        </label>
        <Input
        ...
        />
        <p className="fb-muted" id="nfs-tenant-confirmation-help">
          {Strings.nfsTenantConfirmationHelp}
        </p>
      </section>
      ...
      <section aria-labelledby="nfs-feature-heading" className="fb-nfs-card">
        <div className="fb-nfs-card-heading">
          <div>
            <h3 id="nfs-feature-heading">{Strings.nfsFeature}</h3>
            <p className="fb-muted">{Strings.nfsFeatureHelp}</p>
          </div>
        ...
        <dl className="fb-nfs-generation-grid">
          ...
          <Generation
            Label={Strings.nfsDesiredManifestGeneration}
            Value={Feature.DesiredManifestGeneration}
          />
          <Generation
            Label={Strings.nfsAppliedManifestGeneration}
            Value={Feature.AppliedManifestGeneration}
          />
          <Generation Label={Strings.nfsRestoreGeneration} Value={Feature.RestoreGeneration} />
          <div>
            <dt>{Strings.nfsAppliedGateway}</dt>
            <dd>
              <bdi dir="auto">{Feature.AppliedGatewayId ?? Strings.none}</bdi>
              {Feature.AppliedGatewayEpoch === null
                ? null
                : ` · ${Strings.nfsEpoch} ${Feature.AppliedGatewayEpoch}`}
            </dd>
          </div>
        </dl>
        <FeatureTransitionControls
          Busy={Busy}
          Feature={Feature}
      ...
      <section aria-labelledby="nfs-exports-heading" className="fb-nfs-section">
        <div>
          <h3 id="nfs-exports-heading">{Strings.nfsExports}</h3>
          <p className="fb-muted">{Strings.nfsExportsHelp}</p>
        </div>
          ...
          {Snapshot.Exports.map((Export) => (
            <article className="fb-nfs-card" key={Export.DriveId} role="listitem">
              <div className="fb-nfs-card-heading">
                <div>
                  <strong>
                    <bdi dir="auto">{Export.ExportPath}</bdi>
                  </strong>
                  <p className="fb-muted">
                    {Strings.nfsExportId} {Export.ExportId} · {Strings.nfsDriveId}{" "}
                    <bdi dir="auto">{Export.DriveId}</bdi>
                  </p>
                </div>
                <Badge appearance="tint" color={Export.InSync ? "success" : "warning"}>
                  {Export.InSync ? Strings.nfsApplied : Strings.nfsPending}
                </Badge>
              </div>
              <dl className="fb-nfs-generation-grid">
                <div>
                  <dt>{Strings.nfsDesiredState}</dt>
                  <dd>{Export.DesiredState}</dd>
                </div>
                <Generation Label={Strings.nfsDesiredGeneration} Value={Export.DesiredGeneration} />
                <div>
                  <dt>{Strings.nfsAppliedState}</dt>
                  <dd>{Export.AppliedState}</dd>
                </div>
                <Generation Label={Strings.nfsAppliedGeneration} Value={Export.AppliedGeneration} />
              </dl>
              <ExportTransitionControls
                Busy={Busy}
                Export={Export}
      ...
      <section aria-labelledby="nfs-groups-heading" className="fb-nfs-section">
        <div>
          <h3 id="nfs-groups-heading">{Strings.nfsPosixGroups}</h3>
          <p className="fb-muted">{Strings.nfsGroupsHelp}</p>
        </div>
        <div className="fb-card-list" role="list">
          {Snapshot.PosixGroups.map((Group) => (
            <article className="fb-nfs-card" key={Group.GroupId} role="listitem">
              <strong>
                <bdi dir="auto">{Group.PosixName}</bdi>
              </strong>
              <span>
                {Strings.nfsProjectedGid} {Group.ProjectedGid}
              </span>
              <span className="fb-muted">
                <bdi dir="auto">{Group.GroupId}</bdi>
              </span>
            </article>
          ))}
      ...
      <section aria-labelledby="nfs-proposals-heading" className="fb-nfs-section">
        <div>
          <h3 id="nfs-proposals-heading">{Strings.nfsPendingProposals}</h3>
          <p className="fb-muted">{Strings.nfsPendingProposalsHelp}</p>
        </div>
        <div className="fb-card-list" role="list">
      ...
      <section aria-labelledby="nfs-quarantine-heading" className="fb-nfs-section">
        <div>
          <h3 id="nfs-quarantine-heading">{Strings.nfsQuarantinedMappings}</h3>
          <p className="fb-muted">{Strings.nfsQuarantinedMappingsHelp}</p>
        </div>
        <div className="fb-card-list" role="list">
          {Snapshot.QuarantinedMappings.map((Mapping) => (
            <QuarantinedMappingCard key={Mapping.CredentialId} Mapping={Mapping} />
          ))}
          {Snapshot.QuarantinedMappings.length === 0 ? (
            <p>{Strings.nfsNoQuarantinedMappings}</p>
          ) : null}
        </div>
      </section>
      ...
      <section aria-labelledby="nfs-mappings-heading" className="fb-nfs-section">
        <div>
          <h3 id="nfs-mappings-heading">{Strings.nfsMappings}</h3>
          <p className="fb-muted">{Strings.nfsMappingsHelp}</p>
        </div>
        <div className="fb-card-list" role="list">
      ...
      <section aria-labelledby="nfs-conflicts-heading" className="fb-nfs-section">
        <div>
          <h3 id="nfs-conflicts-heading">{Strings.nfsConflicts}</h3>
          <p className="fb-muted">{Strings.nfsConflictsHelp}</p>
        </div>
        <div className="fb-card-list" role="list">
...
function ReauthenticationNotice(): ReactNode {
  return (
    <div className="fb-mount-reauth" role="alert">
      <p>{Strings.nfsReauthenticationRequired}</p>
      <Button appearance="primary" as="a" href="/api/v1/auth/login?return_path=%2Fadmin">
        {Strings.signInAgain}
      </Button>
    </div>
  );
    ...
    <div className="fb-nfs-confirmation">
      ...
      <div className="fb-nfs-actions">
        {Transitions.map((State) => {
          ...
          return (
            <Button
              appearance={State === "active" ? "primary" : "secondary"}
              disabled={Busy || !MutationEnabled || (RequiresConfirmation && !Confirmed)}
              key={State}
              onClick={() => {
                if (RequiresConfirmation) SetConfirmed(false);
                void OnTransition(Feature.Generation, State);
              }}
            >
...
function ExportTransitionControls({
  ...
  const HasConsequentialTransition = Transitions.some(
    (State) => State === "draining" || State === "disabled",
  );
  if (Transitions.length === 0)
    return <span className="fb-muted">{Strings.nfsNoExportTransition}</span>;
  return (
    <div className="fb-nfs-confirmation">
      {HasConsequentialTransition ? (
        <Checkbox
        ...
        {Transitions.map((State) => {
          ...
          return (
            <Button
              ...
              onClick={() => {
                if (RequiresConfirmation) SetConfirmed(false);
                void OnTransition(Export.DriveId, Export.DesiredGeneration, State);
              }}
            >
              {Strings.nfsTransitionTo(State)}
            </Button>
...
function ExportRegistrationForm({
  ...
  return (
    <form
      aria-labelledby="nfs-register-export-heading"
      className="fb-nfs-card fb-nfs-form"
      onSubmit={Submit}
    >
      <div>
        <h3 id="nfs-register-export-heading">{Strings.nfsRegisterExport}</h3>
        <p className="fb-muted">{Strings.nfsRegisterExportHelp}</p>
      </div>
      <label>
        {Strings.nfsDriveId}
        <Input
...
function PosixGroupRegistrationForm({
  ...
  const Submit = (Event: FormEvent): void => {
    ...
    void OnRegister({
      ...
      PosixName: PosixName.trim(),
      ProjectedGid: ParsedGid,
    });
  };
  return (
    <form
      aria-labelledby="nfs-register-group-heading"
      className="fb-nfs-card fb-nfs-form"
      onSubmit={Submit}
    >
      <div>
        <h3 id="nfs-register-group-heading">{Strings.nfsRegisterPosixGroup}</h3>
        <p className="fb-muted">{Strings.nfsRegisterGroupHelp}</p>
      </div>
      <label>
        {Strings.nfsGroupId}
        <Input
      ...
      <Button
        ...
        disabled={
          ...
          PosixName.trim().length === 0 ||
          ProjectedId(ProjectedGid) === null
        }
        icon={<Plus aria-hidden="true" />}
        type="submit"
      >
...
function MappingProposalForm({
  ...
  return (
    <section aria-labelledby="nfs-map-principal-heading" className="fb-nfs-section">
      <div>
        <h3 id="nfs-map-principal-heading">{Strings.nfsProposeMapping}</h3>
        <p className="fb-muted">
          {Strings.nfsExactRealmHelp} {Strings.nfsConfiguredRealm}: <bdi dir="auto">{Realm}</bdi>.
        </p>
      </div>
      <form className="fb-nfs-card fb-nfs-form" onSubmit={Submit}>
        ...
        <p className="fb-muted" id="nfs-exact-realm-help">
          {Strings.nfsKerberosPrincipalHelp}
        </p>
        <div className="fb-nfs-form-row">
          <label>
            {Strings.nfsProjectedUid}
            <Input
              disabled={Busy}
              inputMode="numeric"
        ...
        <fieldset className="fb-mount-drives">
          ...
          {Exports.map((Export) => (
            <Checkbox
              checked={AllowedDriveIds.has(Export.DriveId)}
              disabled={Busy}
              key={Export.DriveId}
        ...
        <p className="fb-muted" id="nfs-mapping-authority-help">
          <ShieldCheck aria-hidden="true" size={16} /> {Strings.nfsProposalHelp}
        </p>
        <Checkbox
        ...
        <Button
          ...
          disabled={
            ...
            KerberosPrincipal.trim().length === 0 ||
            ProjectedId(ProjectedUid) === null ||
            ProjectedId(ProjectedGid) === null ||
            AllowedDriveIds.size === 0
          }
          type="submit"
        >
          {Strings.nfsCreateProposal}
        </Button>
      </form>
    </section>
  );
...
function MappingCard({
  ...
  const [Confirmed, SetConfirmed] = useState(false);
  const [AllowedDriveIds, SetAllowedDriveIds] = useState<ReadonlySet<string>>(
    () => new Set(Mapping.AllowedDriveIds ?? []),
  );
  const HelpId = `nfs-revoke-${Mapping.CredentialId}-help`;
  const CurrentDriveIds = Mapping.AllowedDriveIds ?? [];
  const IsStrictAttenuation =
    AllowedDriveIds.size > 0 &&
  ...
  return (
    <article className="fb-nfs-card" role="listitem">
      <div className="fb-nfs-card-heading">
        <div>
          <strong>
            <bdi dir="auto">{Mapping.KerberosPrincipal}</bdi>
          </strong>
          <p className="fb-muted">
            {Strings.nfsPrincipalId} <bdi dir="auto">{Mapping.PrincipalId}</bdi>
          </p>
        </div>
        <Badge appearance="tint">
          {Strings.nfsGeneration} {Mapping.Generation}
        </Badge>
      </div>
      <span>
        {Strings.nfsProjectedUid} {Mapping.ProjectedUid} · {Strings.nfsProjectedGid}{" "}
        {Mapping.ProjectedGid}
      </span>
      <span className="fb-muted">
        {Strings.nfsAllowedExports}:{" "}
        {Mapping.AllowedDriveIds === undefined
          ? Strings.nfsAllowedExportsUnknown
          : Mapping.AllowedDriveIds.map((DriveId) => (
              <bdi dir="auto" key={DriveId}>
                {DriveId}{" "}
              </bdi>
            ))}
      </span>
      {Mapping.AllowedDriveIds === undefined ? null : (
        <fieldset className="fb-mount-drives">
          ...
          {Exports.filter(({ DriveId }) => CurrentDriveIds.includes(DriveId)).map((Export) => (
            <Checkbox
              checked={AllowedDriveIds.has(Export.DriveId)}
              disabled={Busy}
              key={Export.DriveId}
          ...
          <Button
            ...
            disabled={Busy || !MutationEnabled || !IsStrictAttenuation}
            onClick={() =>
              void OnAttenuate(Mapping.CredentialId, [...AllowedDriveIds], Mapping.Generation)
            }
          >
            {Strings.nfsSaveAttenuatedScope}
          </Button>
        </fieldset>
      )}
      <p className="fb-muted" id={HelpId}>
        {Strings.nfsRevokeMappingHelp}
      </p>
      <Checkbox
      ...
      <Button
        aria-describedby={HelpId}
        appearance="secondary"
        disabled={Busy || !MutationEnabled || !Confirmed}
        onClick={() => {
          SetConfirmed(false);
          void OnRevoke(Mapping.CredentialId, Mapping.Generation);
        }}
      >
        {Strings.revoke}
      </Button>
  ...
  return (
    <article className="fb-nfs-card" role="listitem">
      <div className="fb-nfs-card-heading">
        <div>
          <strong>
            <bdi dir="auto">{Proposal.KerberosPrincipal}</bdi>
          </strong>
          <p className="fb-muted">
            {Strings.nfsPrincipalId} <bdi dir="auto">{Proposal.PrincipalId}</bdi>
          </p>
        </div>
      ...
      <span>
        {Strings.nfsProjectedUid} {Proposal.ProjectedUid} · {Strings.nfsProjectedGid}{" "}
        {Proposal.ProjectedGid}
      </span>
      <span className="fb-muted">
        {Strings.nfsProposalId} <bdi dir="auto">{Proposal.Id}</bdi> · {Strings.nfsProposedBy}{" "}
        <bdi dir="auto">{Proposal.ProposerPrincipalId}</bdi>
      </span>
      <span className="fb-muted">
        {Strings.nfsAllowedExports}:{" "}
        {Proposal.AllowedDriveIds.map((DriveId) => (
          <bdi dir="auto" key={DriveId}>
            {DriveId}{" "}
          </bdi>
        ))}
      ...
      <span className="fb-muted">
        {Strings.nfsProposalCreatedAt}{" "}
        <time dateTime={Proposal.CreatedAt}>{Proposal.CreatedAt}</time> ·{" "}
        {Strings.nfsProposalExpires} <time dateTime={Proposal.ExpiresAt}>{Proposal.ExpiresAt}</time>{" "}
        · {Strings.nfsGeneration} {Proposal.Generation}
      </span>
      <p className="fb-muted" id={HelpId}>
        {Strings.nfsCancelProposalHelp}
      </p>
      <Checkbox
        aria-describedby={HelpId}
        checked={Confirmed}
      ...
      <Button
        ...
        onClick={() => {
          SetConfirmed(false);
          void OnCancel(Proposal.Id, Proposal.Generation);
        }}
      >
        {Strings.cancel}
      </Button>
...
function QuarantinedMappingCard({ Mapping }: { Mapping: NfsQuarantinedMappingView }): ReactNode {
  return (
    <article className="fb-nfs-card" role="listitem">
      ...
      <span className="fb-muted">
        {Strings.nfsQuarantinedAt}{" "}
        <time dateTime={Mapping.QuarantinedAt}>{Mapping.QuarantinedAt}</time>
      </span>
      <span className="fb-muted">
        {Strings.nfsQuarantineReason}: <bdi dir="auto">{Mapping.QuarantineReason}</bdi>
      </span>
    </article>
  );
}
...
function ConflictCard({
  ...
  const Submit = (Event: FormEvent): void => {
    ...
    if (Generation === null || ParentId.trim().length === 0 || DisplayName.trim().length === 0)
      return;
    void OnCopy({
      DisplayName,
  ...
  return (
    <article className="fb-nfs-card" role="listitem">
      <div className="fb-nfs-card-heading">
        <div>
          <strong>{Strings.nfsConflict}</strong>
          <p className="fb-muted">
            <bdi dir="auto">{Conflict.Id}</bdi>
          </p>
        </div>
        <Badge appearance="tint" color="warning">
          {Strings.nfsRetained}
      ...
      <dl className="fb-nfs-generation-grid">
        <div>
          <dt>{Strings.nfsDriveId}</dt>
          <dd>
            <bdi dir="auto">{Conflict.DriveId}</bdi>
          </dd>
        </div>
        <div>
          <dt>{Strings.nfsSourceNodeId}</dt>
          <dd>
            <bdi dir="auto">{Conflict.SourceNodeId}</bdi>
          </dd>
        </div>
        <div>
          <dt>{Strings.nfsLogicalSize}</dt>
          <dd>{Conflict.LogicalSizeBytes}</dd>
        </div>
        <div>
          <dt>{Strings.nfsConflictExpires}</dt>
          <dd>
            <time dateTime={Conflict.ExpiresAt}>{Conflict.ExpiresAt}</time>
          </dd>
        </div>
      </dl>
      <form aria-describedby={HelpId} className="fb-nfs-form" onSubmit={Submit}>
        <p className="fb-muted" id={HelpId}>
          {Strings.nfsConflictCopyHelp}
        </p>
        <label>
          {Strings.nfsParentId}
          <Input
        ...
        <label>
          {Strings.nfsExpectedParentGeneration}
          <Input
            disabled={Busy}
            inputMode="numeric"
        ...
        <Button
          ...
          disabled={
            ...
            PositiveInteger(ExpectedParentGeneration) === null
          }
          type="submit"
        >
          {Strings.nfsCopyConflict}
        </Button>
      </form>
      <Checkbox
        aria-describedby={HelpId}
      ...
      <Button
        appearance="secondary"
        disabled={Busy || !MutationEnabled || !DiscardConfirmed}
        onClick={() => {
          SetDiscardConfirmed(false);
          void OnDiscard();
        }}
      >
        {Strings.nfsDiscardConflict}
      </Button>
    </article>
  );
}
