...
export function IsFileBeltOfficeAstV1(Value: unknown): Value is FileBeltOfficeAstV1 {
  if (!IsRecord(Value) || !HasOnly(Value, ["Children", "Profile", "Range"]) || Value.Profile !== "filebelt-gfm-v1" || !Array.isArray(Value.Children) || !IsRange(Value.Range)) return false;
...
function IsBlock(Value: unknown, Budget: Budget): Value is OfficeBlock {
  ...
  switch (Value.Kind) {
    case "alert":
      Valid = HasOnly(Value, ["Children", "Kind", "Range", "Severity"])
        && IsOneOf(Value.Severity, ["caution", "important", "note", "tip", "warning"])
        && IsArrayOf(Value.Children, (Child) => IsBlock(Child, Nested));
      break;
    case "code":
      Valid = HasOnly(Value, ["Code", "Kind", "Language", "Range"])
        && TakeString(Value.Code, Budget)
        && (Value.Language === null || TakeString(Value.Language, Budget, 256));
      break;
    case "footnoteDefinition":
      Valid = HasOnly(Value, ["Children", "Identifier", "Kind", "Range"])
        && TakeString(Value.Identifier, Budget, 1_024)
        && IsArrayOf(Value.Children, (Child) => IsBlock(Child, Nested));
      break;
    case "heading":
      Valid = HasOnly(Value, ["Children", "Depth", "Kind", "Range"])
        && Number.isInteger(Value.Depth) && Number(Value.Depth) >= 1 && Number(Value.Depth) <= 6
        && IsArrayOf(Value.Children, (Child) => IsInline(Child, Nested));
      break;
    case "list":
      Valid = HasOnly(Value, ["Items", "Kind", "Ordered", "Range"])
        && typeof Value.Ordered === "boolean"
        && IsArrayOf(Value.Items, (Item) => IsListItem(Item, Nested));
      break;
    case "math":
      Valid = HasOnly(Value, ["Expression", "Kind", "Range"])
        && TakeString(Value.Expression, Budget, 64 * 1_024);
      break;
    case "mermaid":
      Valid = HasOnly(Value, ["Kind", "Range", "Source"])
        && TakeString(Value.Source, Budget, 64 * 1_024);
      break;
    case "paragraph":
      Valid = HasOnly(Value, ["Children", "Kind", "Range"])
        && IsArrayOf(Value.Children, (Child) => IsInline(Child, Nested));
      break;
    case "quote":
      Valid = HasOnly(Value, ["Children", "Kind", "Range"])
      ...
      break;
    case "table":
      Valid = HasOnly(Value, ["Align", "Kind", "Range", "Rows"])
        && Array.isArray(Value.Align) && Value.Align.length <= 1_024
        && Value.Align.every((Align) => Align === null || IsOneOf(Align, ["center", "left", "right"]))
...
function IsInline(Value: unknown, Budget: Budget): Value is OfficeInline {
  ...
  switch (Value.Kind) {
    ...
    case "strong":
      ...
      break;
    case "filebeltLink":
      Valid = HasOnly(Value, ["Kind", "Range", "Target", "Title"])
        && IsFileBeltReference(Value.Target)
        && (Value.Title === null || TakeString(Value.Title, Budget, 1_024));
      break;
    case "footnoteReference":
      Valid = HasOnly(Value, ["Identifier", "Kind", "Range"])
        && TakeString(Value.Identifier, Budget, 1_024);
      break;
    case "link":
      Valid = HasOnly(Value, ["Children", "Destination", "Kind", "Range", "Title"])
...
function IsListItem(Value: unknown, Budget: Budget): boolean {
  if (!Enter(Value, Budget) || !HasOnly(Value, ["Checked", "Children", "Range"]) || !IsRange(Value.Range) || !(Value.Checked === null || typeof Value.Checked === "boolean")) return false;
  const Nested = { ...Budget, Depth: Budget.Depth + 1 };
...
function IsTableRow(Value: unknown, Budget: Budget): boolean {
  if (!Enter(Value, Budget) || !HasOnly(Value, ["Cells", "Range"]) || !IsRange(Value.Range) || !Array.isArray(Value.Cells) || Value.Cells.length > 1_024) return false;
  const Nested = { ...Budget, Depth: Budget.Depth + 1 };
...
function IsFileBeltReference(Value: unknown): boolean {
  return IsRecord(Value)
    && HasOnly(Value, Value.VersionId === undefined ? ["DriveId", "NodeId"] : ["DriveId", "NodeId", "VersionId"])
    && IsUuid(Value.DriveId) && IsUuid(Value.NodeId)
    && (Value.VersionId === undefined || IsUuid(Value.VersionId));
}

function IsRange(Value: unknown): Value is SourceRange {
  return IsRecord(Value) && HasOnly(Value, ["End", "Start"])
