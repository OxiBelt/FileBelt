...
export type OfficeImportSourceType = "csv" | "docx" | "odp" | "ods" | "odt" | "pptx" | "rtf" | "xlsx";
...
type OfficeParserResult = Record<"messages", readonly OfficeParserMessage[]> & Record<"value", string>;
...
export interface OfficeImportModule {
  convert(Contents: Uint8Array, Destination: "md", Options: OfficeParserOptions): Promise<OfficeParserResult>;
}
...
export function CreateMermaidRenderBudget(MaximumDiagrams = 50): MermaidRenderBudget {
  ...
  return {
    Reserve(): void {
      Rendered += 1;
      if (Rendered > MaximumDiagrams) throw new RangeError("Markdown preview exceeds 50 Mermaid diagrams.");
    },
  };
}

export async function RenderMermaid(LoadMermaid: () => Promise<MermaidModule>, Options: MermaidRenderOptions, Budget = CreateMermaidRenderBudget()): Promise<string> {
  if (new TextEncoder().encode(Options.Source).byteLength > 64 * 1024) throw new RangeError("Mermaid source exceeds 64 KiB.");
  if (CountMermaidEdges(Options.Source) > 500) throw new RangeError("Mermaid diagram exceeds 500 edges.");
  if (/^\s*click\b/im.test(Options.Source)) throw new RangeError("Mermaid click directives are not permitted.");
  Budget.Reserve();
  const Mermaid = (await LoadMermaid()).default;
  if (Mermaid === undefined) throw new Error("Mermaid module has no default export.");
  Mermaid.initialize({ flowchart: { htmlLabels: false }, securityLevel: "strict", startOnLoad: false });
  return (await Mermaid.render(Options.DiagramId, Options.Source)).svg;
}

export async function RenderKaTeX(LoadKaTeX: () => Promise<KaTeXModule>, Expression: string): Promise<string> {
...
export async function ImportOfficeMarkdown(
  ...
  if (Options.Contents.byteLength > MaximumInputBytes) throw new RangeError("Office import exceeds the 8 MiB browser input limit.");
  const Result = await (await LoadOfficeImporter()).convert(Options.Contents, "md", {
    generatorConfig: { ignoreInternalLinks: true, includeCharts: false, includeImages: false },
    parseConfig: {
      ...(Options.Signal === undefined ? {} : { abortSignal: Options.Signal }),
      decompressionLimits: { maxTableCells: 100_000, maxUncompressedBytes: 64 * 1024 * 1024, maxZipEntries: 4_096 },
  ...
  if (typeof Result.value !== "string" || Result.messages.length > 0) {
    throw new Error(Result.messages[0]?.message ?? "The Office document could not be converted safely.");
  }
  if (Result.value.includes("\0")) throw new Error("Converted Markdown contains a NUL byte.");
  if (new TextEncoder().encode(Result.value).byteLength > MaximumOutputBytes) throw new RangeError("Converted Markdown exceeds the 2 MiB editor limit.");
  return Result.value;
}
...
function IsOfficeImportSourceType(Value: string): Value is OfficeImportSourceType {
  return Value === "csv" || Value === "docx" || Value === "odp" || Value === "ods" || Value === "odt" || Value === "pptx" || Value === "rtf" || Value === "xlsx";
}
