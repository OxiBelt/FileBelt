...
describe("filebelt-gfm-v1 normalization", () => {
  it("keeps source ranges and converts permitted extensions to Office AST", () => {
    ...
    expect(Result.Ast.Children.map((Block) => Block.Kind)).toEqual(["heading", "alert", "mermaid", "math", "footnoteDefinition"]);
    ...
    expect(Result.Diagnostics.some((Diagnostic) => Diagnostic.Code === "markdown.raw-html")).toBe(true);
  });
  ...
  it("fails closed before recursively normalizing an excessive AST", () => {
    ...
    const Result = NormalizeMdast(Root as never, { HasByteOrderMark: false, LineEnding: "none", Text: "deep" });
