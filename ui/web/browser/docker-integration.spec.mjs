...
const ComposeFiles = (process.env.FILEBELT_ACCEPTANCE_COMPOSE_FILES ?? "")
  .split(":")
  .filter(Boolean);
  ...
  return Page.evaluate(
    async ({ Drive, Node }) => {
      ...
      const GrantResponse = await fetch(
        `/api/v1/drives/${Drive}/nodes/${Node}/collaboration-grants`,
        {
          body: JSON.stringify({
            client_id: crypto.randomUUID(),
            presence_mode: "display_name",
            transport: "websocket",
          }),
      ...
      const Field = (NumberValue, Bytes) => [
        ...Varint((NumberValue << 3) | 2),
        ...Varint(Bytes.length),
        ...Bytes,
      ];
        ...
        new Promise((Resolve) => {
          ...
          Socket.addEventListener(
            ...
            (Event) => {
              ...
              const Outcome =
                Bytes[0] === 0x1a ? "sync" : Bytes[0] === 0x4a ? "rejected" : `frame:${Bytes[0]}`;
              Socket.close();
              Resolve(Outcome);
            },
            { once: true },
  ...
  return Page.evaluate(
    async ({ Drive, Node }) => {
      ...
      const Allocation = await Json(
        await fetch(`/api/v1/drives/${Drive}/uploads`, {
          body: JSON.stringify({
      ...
      const Committed = await Json(
        await fetch(`/api/v1/uploads/${Allocation.upload_id}/commit`, {
...
test("converges, checkpoints, reconnects, and revokes a two-user room", async ({
  ...
  const ShareStatus = await Admin.evaluate(
    async ({ Drive, Node, Csrf }) => {
  ...
  await expect(Admin.getByText("Live collaboration disconnected.")).toBeVisible({
    timeout: 30_000,
  });
...
test("freezes dirty collaboration on an external head and surfaces a conflict copy", async ({
  browser: BrowserValue,
  browserName: BrowserName,
}) => {
  ...
  await expect(Admin.getByText("Live collaboration connected.")).toBeVisible();
  await expect(Member.getByRole("textbox", { name: "Markdown source" })).toContainText(
    "local dirty",
  );
