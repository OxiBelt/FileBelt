...
function Node(
  ...
  return {
    acl_generation: 2,
    attribute_generation: 3,
    content_class_policy: "auto",
    display_name: Kind === "directory" ? "Documents" : "Plan.docx",
    drive_id: DriveId,
    head_media_type:
      Kind === "directory"
        ? null
        : "application/vnd.openxmlformats-officedocument.wordprocessingml.document",
    head_version_id: Kind === "directory" ? null : VersionId,
    id: Id,
    kind: Kind,
    namespace_generation: 7,
    parent_id: Parent,
    size_bytes: Kind === "directory" ? null : 8,
    trashed: false,
    updated_at: "2026-08-09T10:00:00Z",
    version_ordinal: Kind === "directory" ? null : 1,
  };
}
...
describe("HttpDocumentSessionClient", () => {
  it("uses generated session routes with in-memory CSRF and idempotency headers", async () => {
    ...
    const Fetch: typeof fetch = async (Input, Init) => {
      ...
      if (
        Path === `/api/v1/drives/${DriveId}/nodes/${NodeId}/document-sessions` &&
        HttpRequest.method === "POST"
      )
        return Json(Detail, 201);
      if (Path === "/api/v1/document-sessions")
        return Json({ items: [Summary], next_cursor: null });
      if (Path === `/api/v1/document-sessions/${SessionId}` && HttpRequest.method === "GET")
        return Json(Detail);
      if (Path === `/api/v1/document-sessions/${SessionId}` && HttpRequest.method === "DELETE")
        return new Response(null, { status: 204 });
      if (
        Path === `/api/v1/document-sessions/${SessionId}/handoff` &&
        HttpRequest.method === "POST"
      )
        return Json(
          {
            action: "https://filebelt.localhost/onlyoffice/launch",
            expires_at: "2026-08-09T10:16:00Z",
            grant: "one-use-grant-value-long-enough-for-the-contract",
            session_id: SessionId,
          },
          201,
        );
      if (Path === `/api/v1/drives/${DriveId}/nodes/${NodeId}` && HttpRequest.method === "GET")
        return Json(Node(NodeId, ParentId, "file"));
      if (Path === `/api/v1/drives/${DriveId}/nodes/${ParentId}` && HttpRequest.method === "GET")
        return Json(Node(ParentId, null, "directory"));
      if (
        Path === `/api/v1/document-sessions/${SessionId}/conflict-copy` &&
        HttpRequest.method === "POST"
      )
        return Json(
          {
            node: Node("00000000-0000-4000-8000-000000000009", ParentId, "file"),
            version: {
              created_at: "2026-08-09T10:00:00Z",
      ...
      if (
        Path === `/api/v1/drives/${DriveId}/nodes/${NodeId}/document-sessions/${SessionId}` &&
    ...
    const Create = Find(
      Requests,
      "POST",
      `/api/v1/drives/${DriveId}/nodes/${NodeId}/document-sessions`,
    );
    ...
    expect(await Copy.clone().json()).toEqual({
      expected_parent_generation: 7,
      target_name: "Plan (conflicted copy).docx",
      target_parent_id: ParentId,
    });
    expect(
      Find(Requests, "POST", `/api/v1/document-sessions/${SessionId}/handoff`).headers.get(
        "idempotency-key",
      ),
    ).toBeNull();
