...
export class HttpMcpSettingsClient implements McpSettingsClient {
  ...
  async getSnapshot(IsTenantAdmin: boolean, Signal?: AbortSignal): Promise<McpSettingsSnapshot> {
    ...
    const [Registrations, Activity, Templates, Services, BlockRules] = await Promise.all([
      ...
      IsTenantAdmin ? this.#Api.GET("/api/v1/admin/mcp/templates", { params: { query: PageQuery }, ...SignalInit }) : null,
      IsTenantAdmin ? this.#Api.GET("/api/v1/admin/mcp/service-identities", { params: { query: PageQuery }, ...SignalInit }) : null,
    ...
    return {
      ...
      BlockRules: BlockRules === null ? [] : RequireData<readonly AdminBlockRuleResponse[]>(BlockRules).map(BlockRuleView),
      Registrations: RequireData<RegistrationPage>(Registrations).items.map(RegistrationView),
      ServiceIdentities: Services === null ? [] : RequireData<AdminServicePage>(Services).items.map(ServiceView),
      Templates: Templates === null ? [] : RequireData<AdminTemplatePage>(Templates).items.map(TemplateView),
    };
  }

  async createRegistration(Input: CreateMcpRegistrationInput): Promise<void> {
    RequireSuccess(await this.#Api.POST("/api/v1/mcp/registrations", {
      body: {
        attachment_policy: {
  ...
  async importRegistration(Document: string): Promise<void> {
    ...
    RequireSuccess(await this.#Api.POST("/api/v1/mcp/registrations/import", {
  ...
  async exportRegistration(RegistrationId: string): Promise<string> {
    const Value = RequireData<components["schemas"]["McpRegistrationExport"]>(await this.#Api.GET("/api/v1/mcp/registrations/{registration_id}/export", {
  ...
  async deleteRegistration(Registration: McpRegistrationView): Promise<void> {
    RequireSuccess(await this.#Api.DELETE("/api/v1/mcp/registrations/{registration_id}", {
      params: { header: await this.#versionedHeaders(Registration.Etag), path: { registration_id: Registration.Id } },
    }));
  }

  async changeRegistrationState(Registration: McpRegistrationView, Action: "disable" | "enable" | "revoke"): Promise<void> {
    RequireSuccess(await this.#Api.POST("/api/v1/mcp/registrations/{registration_id}/state", {
      body: { action: Action },
      params: { header: await this.#versionedHeaders(Registration.Etag), path: { registration_id: Registration.Id } },
    }));
  }
  ...
  async testRegistration(Registration: McpRegistrationView): Promise<boolean> {
    const Result = RequireData<components["schemas"]["McpTestResult"]>(await this.#Api.POST("/api/v1/mcp/registrations/{registration_id}/test", {
      params: { header: await this.#versionedHeaders(Registration.Etag), path: { registration_id: Registration.Id } },
    }));
    return Result.succeeded;
  }

  async putCredential(Registration: McpRegistrationView, Kind: "api_key" | "bearer", Secret: string): Promise<void> {
    RequireSuccess(await this.#Api.PUT("/api/v1/mcp/registrations/{registration_id}/credentials", {
  ...
  async startOauth(Registration: McpRegistrationView): Promise<string> {
    const Result = RequireData<components["schemas"]["McpOauthStart"]>(await this.#Api.POST("/api/v1/mcp/registrations/{registration_id}/oauth/start", {
  ...
  async getCapabilityReview(RegistrationId: string): Promise<McpCapabilityReviewView | null> {
    const Result = await this.#Api.GET("/api/v1/mcp/registrations/{registration_id}/capability-review", {
      params: { path: { registration_id: RegistrationId } },
    });
  ...
  async discoverCapabilities(Registration: McpRegistrationView): Promise<McpCapabilityReviewView> {
    const Snapshot = RequireData<CapabilitySnapshotResponse>(await this.#Api.POST("/api/v1/mcp/registrations/{registration_id}/discover", {
      params: { header: await this.#versionedHeaders(Registration.Etag), path: { registration_id: Registration.Id } },
    }));
    return CapabilityReviewView({ decisions: [], reviewed_at: null, snapshot: Snapshot });
  }

  async putCapabilityReview(Registration: McpRegistrationView, Review: McpCapabilityReviewView): Promise<void> {
    RequireSuccess(await this.#Api.PUT("/api/v1/mcp/registrations/{registration_id}/capability-review", {
      body: {
        decisions: Review.Capabilities.map(({ Fingerprint }) => ({ capability_fingerprint: Fingerprint, decision: Review.Decisions[Fingerprint] ?? "blocked" })),
  ...
  async createInvocationIntent(Input: McpInvocationInput): Promise<McpPreparedInvocation> {
    ...
    const Intent = RequireData<InvocationIntent>(await this.#Api.POST("/api/v1/mcp/invocation-intents", {
      body: Body,
      params: { header: await this.#mutationHeaders() },
    }));
    return { Input, Intent: { ExpiresAt: Intent.expires_at, Id: Intent.id, RequestDigest: Intent.request_digest } };
  }

  async approveAndInvoke(Prepared: McpPreparedInvocation, OnEvent: (Event: McpInvocationEventView) => void, Signal?: AbortSignal): Promise<void> {
    const Body = InvocationRequest(Prepared.Input);
    RequireSuccess(await this.#Api.POST("/api/v1/mcp/invocation-intents/{intent_id}/approval", {
    ...
    const Response = await this.#Fetch(new Request(new URL(`/api/v1/mcp/invocation-intents/${Prepared.Intent.Id}/stream`, this.#BaseUrl), {
    ...
    if (Pending.trim().length > 0) OnEvent(InvocationEventView(JSON.parse(Pending) as InvocationEvent));
  }

  async cancelInvocation(InvocationId: string): Promise<void> {
    RequireSuccess(await this.#Api.DELETE("/api/v1/mcp/invocations/{invocation_id}", {
      params: { header: await this.#mutationHeaders(), path: { invocation_id: InvocationId } },
    }));
  }

  async createTemplate(DisplayName: string, EndpointUri: string, TrustProfile: string): Promise<void> {
    RequireSuccess(await this.#Api.POST("/api/v1/admin/mcp/templates", {
      body: { display_name: DisplayName, endpoint_uri: EndpointUri, transport: "streamable_http", trust_profile: TrustProfile },
      params: { header: await this.#mutationHeaders() },
    }));
  }

  async assignTemplate(Template: AdminMcpTemplateView, PrincipalId: string, PrincipalKind: "group" | "service" | "user"): Promise<void> {
    RequireSuccess(await this.#Api.PUT("/api/v1/admin/mcp/templates/{template_id}/assignments/{principal_id}", {
  ...
  async createServiceIdentity(DisplayName: string, SpiffeUri: string): Promise<void> {
    RequireSuccess(await this.#Api.POST("/api/v1/admin/mcp/service-identities", {
  ...
  async createServiceInvocationGrant(
    ...
    RequireSuccess(await this.#Api.POST("/api/v1/admin/mcp/service-identities/{service_id}/invocation-grants", {
      body: {
        application_id: ApplicationId,
        argument_constraints: {},
        capability: { fingerprint: CapabilityFingerprint, kind: CapabilityKind, name: CapabilityName },
        expires_at: ExpiresAt,
        max_invocations_per_hour: 60,
        mcp_data_grant_ids: [],
        registration_id: RegistrationId,
      },
      params: { header: await this.#versionedHeaders(Service.Etag), path: { service_id: Service.Id } },
    }));
  }

  async createBlockRule(Kind: AdminMcpBlockRuleView["Kind"], Value: string, Reason: string): Promise<void> {
    RequireSuccess(await this.#Api.POST("/api/v1/admin/mcp/block-rules", {
  ...
  async #ensureSession(Signal?: AbortSignal): Promise<void> {
    ...
    const Session = RequireData<SessionResponse>(await this.#Api.GET("/api/v1/session", Signal === undefined ? {} : { signal: Signal }));
  ...
  async #streamHeaders(): Promise<Omit<MutationHeaders, "Idempotency-Key">> {
    ...
    return { Origin: Headers.Origin, "Sec-Fetch-Site": Headers["Sec-Fetch-Site"], "X-FileBelt-Csrf": Headers["X-FileBelt-Csrf"] };
  }
}
...
function CapabilityReviewView(Value: CapabilityReviewResponse): McpCapabilityReviewView {
  return {
    ...
    Decisions: Object.fromEntries(Value.decisions.map((Decision) => [Decision.capability_fingerprint, Decision.decision])),
...
function ActivityView(Value: components["schemas"]["McpActivity"]): McpActivityView {
  return { ApplicationId: Value.application_id, CapabilityFingerprint: Value.capability_fingerprint, CreatedAt: Value.created_at, DurationMs: Value.duration_ms, Id: Value.id, Outcome: Value.outcome, ReasonCode: Value.reason_code, RegistrationId: Value.registration_id };
}

function TemplateView(Value: components["schemas"]["AdminMcpTemplate"]): AdminMcpTemplateView {
  return { AssignmentCount: Value.assignment_count, DisplayName: Value.display_name, Enabled: Value.enabled, EndpointUri: Value.endpoint_uri, Etag: Value.etag, Id: Value.id, Transport: Value.transport, TrustProfile: Value.trust_profile };
}

function ServiceView(Value: components["schemas"]["AdminMcpServiceIdentity"]): AdminMcpServiceIdentityView {
  return { DisplayName: Value.display_name, Etag: Value.etag, Id: Value.id, SpiffeUri: Value.spiffe_uri, State: Value.state };
}
...
function RegistrationExport(Value: unknown): components["schemas"]["McpRegistrationExport"] {
  if (typeof Value !== "object" || Value === null || !("format" in Value) || Value.format !== "filebelt.mcp-registration.v1") {
    throw new Error("The MCP registration JSON is invalid.");
  }
...
function InvocationEventView(Value: InvocationEvent): McpInvocationEventView {
  ...
  if (Value.event === "error") return { Kind: "error", ProblemCode: Value.problem_code ?? "mcp.unknown" };
  if (Value.event === "progress") return Value.progress === null || Value.progress === undefined ? { Kind: "progress" } : { Kind: "progress", Progress: Value.progress };
...
function RequireSuccess(Result: ApiResult<unknown>): void {
  if (!Result.response.ok) throw new Error(`FileBelt MCP request failed (${Result.response.status}).`);
}
